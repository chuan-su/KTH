...
public class Jabeja {
  ...
  private void sampleAndSwap(int nodeId) {
    Node partner = null;
    Node nodep = entireGraph.get(nodeId); //node p
    ...
    if (
     (config.getNodeSelectionPolicy() == NodeSelectionPolicy.HYBRID && partner == null)
  ...
  //T is global variable, dont need as function parameter
  public Node findPartner(int nodeId, Integer[] nodes){
    ...
    for (int id: nodes) {
        ...
        double oldD = Math.pow(d_pp,alpha)+Math.pow(d_qq,alpha);

        int d_pq = getDegree(nodep,nodeq.getColor());
        int d_qp = getDegree(nodeq,nodep.getColor());
        double newD = Math.pow(d_pq,alpha)+Math.pow(d_qp,alpha);

...
species FestivalGuestMemory skills: [moving] {
	float size <- 1#m;
	float speed <- 2#km/#h;
	rgb color <- #black;
	...
	reflex goto_infoCenter when:infoCenter != nil and store = nil {
		...
		total_distance_memory <- total_distance_memory+1;
	}
...
experiment main type: gui {
	output {
		display map type: opengl {
			...
			species FestivalGuestMemory aspect: base;
		...
		display chart {
			chart "Festival guest information"{
				data "Distance traveled wihtout memory" value: total_distance color: #red;
				data "Distance traveled with memory" value: total_distance_memory color: #blue;
			}
		}
	}
